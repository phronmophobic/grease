...
static id GreaseJSONObjectOrNull(id value) {
...
+ (NSString *)bootstrapJavaScript
{
    ...
    @"function newId(){var parts=nextId.split('-');var n=Number(parts[1])+1;nextId=parts[0]+'-'+n;return nextId;}"
    ...
    @"  else{var message=(response.value&&response.value.message)||response.status||'Native bridge error';"
    @"       var error=new Error(message);error.status=response.status;error.value=response.value;callback.reject(error);}"
    @"}"
    @"function exec(service,action,args){"
    @"  return new Promise(function(resolve,reject){"
    @"    if(!window.webkit||!window.webkit.messageHandlers||!window.webkit.messageHandlers.greaseBridge){"
    @"      reject(new Error('Grease native bridge is unavailable'));return;"
    @"    }"
    @"    var id=newId();"
    @"    callbacks[id]={resolve:resolve,reject:reject};"
    @"    window.webkit.messageHandlers.greaseBridge.postMessage({id:id,service:service,action:action,args:args||[]});"
    @"  });"
    @"}"
...
- (NSDictionary *)responseForScriptMessageBody:(id)body
{
    if (![body isKindOfClass:NSDictionary.class]) {
        return [self responseWithId:nil ok:NO status:@"bad_request" value:@{@"message": @"Bridge message must be an object"} keepCallback:NO];
    }

    NSDictionary *message = (NSDictionary *)body;
    id callbackId = message[@"id"];
    NSString *service = [message[@"service"] isKindOfClass:NSString.class] ? message[@"service"] : nil;
    ...
    if (![callbackId isKindOfClass:NSString.class] && ![callbackId isKindOfClass:NSNumber.class]) {
        return [self responseWithId:nil ok:NO status:@"bad_request" value:@{@"message": @"Bridge message id must be a string or number"} keepCallback:NO];
    }

    if (service.length == 0 || action.length == 0) {
        return [self responseWithId:callbackId ok:NO status:@"bad_request" value:@{@"message": @"Bridge message requires service and action"} keepCallback:NO];
    }

    if (args != nil && ![args isKindOfClass:NSArray.class]) {
        return [self responseWithId:callbackId ok:NO status:@"bad_request" value:@{@"message": @"Bridge args must be an array"} keepCallback:NO];
    }

    if (![service isEqualToString:@"Device"]) {
        return [self responseWithId:callbackId ok:NO status:@"service_not_found" value:@{@"message": @"Unknown bridge service"} keepCallback:NO];
    }

    if (![action isEqualToString:@"getInfo"]) {
        return [self responseWithId:callbackId ok:NO status:@"action_not_found" value:@{@"message": @"Unknown Device action"} keepCallback:NO];
    }

    return [self responseWithId:callbackId ok:YES status:@"ok" value:[self deviceInfo] keepCallback:NO];
}
...
- (NSDictionary *)responseWithId:(id)callbackId ok:(BOOL)ok status:(NSString *)status value:(id)value keepCallback:(BOOL)keepCallback
{
    return @{
